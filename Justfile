...
# Authenticated HTTP, durable SQLite, actor, and idempotency boundaries.
bench-managed:
    cargo bench -p nanocentaur --bench managed_api
...
prepare-browser-vm-image firmware=".cache/libkrunfw/libkrunfw" cache=".cache/browser-vm":
    ...
    cargo run --release -p nanocodex-examples --bin browser-vm-image -- \
        crates/experimental/nanocodex-browser-vm/image \
...
release-check version:
    ...
    @diff -u \
        <(sort scripts/release-crates.txt) \
...
# Compare the current Rust test inventory with the pinned pre-refactor master.
parity-audit baseline="ad2952b9c8a4e6946440c7501783e814fa72a215":
    ./scripts/audit-master-tests.sh "{{baseline}}"
