...
use clap::ValueEnum;
use nanocodex::{Model, OpenAi, Thinking, Tools};
...
/// Cloneable Nanocodex recipe shared by every pane in one TUI runtime.
...
impl fmt::Debug for TuiAgentRuntime {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("TuiAgentRuntime")
...
impl TuiAgentRuntime {
    pub(crate) fn openai(&self) -> OpenAi {
        self.openai.clone()
    }
...
/// Authentication method used by Nanocodex.
...
pub(crate) struct Config {
    ...
    theme: Theme,
    #[serde(skip)]
    runtime: Option<Arc<TuiAgentRuntime>>,
    #[serde(skip)]
    reload: ReloadSource,
}
...
impl Config {
    ...
    fn load_with_options(
        ...
        Ok(Self {
            ...
            skills,
            memory: MemoryConfig {
                enabled: overrides.memory.unwrap_or(file.memory.enabled),
            },
            subagents: SubagentsConfig {
                enabled: overrides
                    .subagents
                    .or(file.subagents.enabled)
                    .unwrap_or(true),
            },
            theme: file.theme,
            runtime: None,
    ...
    pub(crate) fn reload(&self) -> Result<ConfigReload> {
        ...
        config.runtime.clone_from(&self.runtime);
        Ok(ConfigReload {
            config,
            workspace_changed,
        })
    ...
    pub(crate) fn install_runtime(&mut self, runtime: Arc<TuiAgentRuntime>) {
        self.runtime = Some(runtime);
    }

    pub(crate) fn runtime(&self) -> Option<&TuiAgentRuntime> {
    ...
    fn write_document(path: &Path, document: DocumentMut) -> Result<()> {
        ...
        // guarantees therefore cover the parsed CLI values owned by Nanocodex, not toml_edit's
...
impl Environment {
    fn read() -> Self {
        Self {
            nanocodex_home: Self::non_empty_var("NANOCODEX_HOME").map(PathBuf::from),
...
impl Config {
    fn config_path(
        ...
        if let Some(home) = &environment.nanocodex_home {
        ...
        environment
            .home
            .as_ref()
            .map(|home| home.join(".nanocodex/config.toml"))
...
mod tests {
    ...
    fn missing_default_file_materializes_all_defaults() {
        ...
        assert_eq!(config.path(), home.join(".nanocodex/config.toml"));
    ...
    #[test]
    fn nanocodex_home_selects_the_config_path() {
        let directory = tempdir().unwrap();
        let nanocodex_home = directory.path().join("nanocodex-home");
        let config = Config::load_with(
            ConfigOverrides::default(),
            Environment {
                nanocodex_home: Some(nanocodex_home.clone()),
        ...
        assert_eq!(config.path(), nanocodex_home.join("config.toml"));
    }
