...
pub(crate) enum AuthError {
    ...
    ApiKeyUnavailable,
    #[error(
        "no ChatGPT credentials found at {path} and OPENAI_API_KEY is not set; run `nanocodex auth login` or set OPENAI_API_KEY"
    )]
...
pub(crate) enum ConfigError {
    #[error("could not determine the config directory; set NANOCODEX_HOME or pass --config")]
