...
pub(crate) fn current() -> &'static InstallationKind {
    INSTALLATION.get_or_init(|| {
        let explicitly_released = matches!(env!("NANOCODEX_RELEASE_BUILD"), "true");
...
fn crates_io_install_root(
    ...
    match cargo_metadata_nanocodex_ownership(&root) {
        Some(true) => return Some(root),
        Some(false) => return None,
        None => {}
    }
    cargo_list_owns_nanocodex(&installed_packages(&root)?).then_some(root)
}
...
fn cargo_metadata_nanocodex_ownership(root: &Path) -> Option<bool> {
    ...
    Some(metadata.v1.iter().any(|(package, binaries)| {
        crates_io_nanocodex_package(package) && binaries.iter().any(|binary| binary == "nanocodex")
    }))
}

fn crates_io_nanocodex_package(package: &str) -> bool {
    let Some(package) = package.strip_prefix("nanocodex ") else {
        return false;
    };
...
fn candidate_cargo_install_root(executable: &Path) -> Option<PathBuf> {
    ...
    if !matches!(filename, "nanocodex" | "nanocodex.exe") {
        return None;
    }
...
fn cargo_list_owns_nanocodex(output: &str) -> bool {
    ...
    while let Some(header) = lines.next() {
        ...
        let is_registry_nanocodex = fields.next() == Some("nanocodex")
            && fields
                .next()
        ...
        {
            owns_binary |= lines.next().is_some_and(|line| line.trim() == "nanocodex");
        }
        if is_registry_nanocodex && owns_binary {
            return true;
        }
    }
    false
...
mod tests {
    use super::{
        InstallationKind, candidate_cargo_install_root, cargo_list_owns_nanocodex,
        cargo_metadata_nanocodex_ownership, detect,
    };
    use std::{
        fs,
        path::{Path, PathBuf},
    ...
    fn registry_metadata_identifies_a_crates_io_installation() {
        ...
        let executable = root.path().join("bin/nanocodex");
        fs::create_dir(root.path().join("bin")).unwrap();
        fs::write(
            root.path().join(".crates.toml"),
            r#"[v1]
"nanocodex 1.2.3 (registry+https://github.com/rust-lang/crates.io-index)" = ["nanocodex"]
...
"#,
    ...
    fn release_archives_and_repository_builds_are_distinct() {
        assert_eq!(
            detect(
                true,
                Some(Path::new("/work/target/release/nanocodex")),
                |_| None
            ),
        ...
        assert_eq!(
            detect(
                false,
                Some(Path::new("/work/target/debug/nanocodex")),
                |_| None
            ),
            InstallationKind::Development,
        );
    ...
    #[test]
    fn cargo_list_only_matches_nanocodex_owning_the_nanocodex_binary() {
        assert!(cargo_list_owns_nanocodex(
            "bat v0.26.1:\n    bat\nnanocodex v1.2.3:\n    nanocodex\n"
        ));
        assert!(!cargo_list_owns_nanocodex(
            "nanocodex v1.2.3 (/work/nanocodex):\n    nanocodex\n"
        ));
        assert!(!cargo_list_owns_nanocodex(
            "nanocodex v1.2.3 (git+https://example.com/nanocodex):\n    nanocodex\n"
        ));
        assert!(!cargo_list_owns_nanocodex(
            "nanocodex v1.2.3:\n    helper\n"
        ));
    ...
    fn registry_metadata_rejects_non_crates_io_installations() {
        for source in [
            "path+file:///work/nanocodex",
            "git+https://example.com/nanocodex",
            "registry+https://example.com/index",
        ] {
            let root = tempfile::tempdir().unwrap();
            fs::write(
                root.path().join(".crates.toml"),
                format!("[v1]\n\"nanocodex 1.2.3 ({source})\" = [\"nanocodex\"]\n"),
            )
            .unwrap();

            assert_eq!(cargo_metadata_nanocodex_ownership(root.path()), Some(false));
        }
    }
    ...
    fn cargo_install_root_requires_the_expected_bin_layout() {
        assert_eq!(
            candidate_cargo_install_root(Path::new("/opt/tools/bin/nanocodex")),
            Some(PathBuf::from("/opt/tools"))
        );
        assert_eq!(
            candidate_cargo_install_root(Path::new("/work/target/debug/nanocodex")),
            None
        );
        assert_eq!(
