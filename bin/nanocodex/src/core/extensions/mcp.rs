    ...
    for (name, config) in config.mcp_servers() {
        let server = match config {
            ...
            McpServerConfig::Http(config) => {
                ...
                // Nanocodex zeroizes each transient environment read after Nanocodex copies it.
                // Nanocodex's retained, non-zeroizing copy is outside this module's secret ownership.
