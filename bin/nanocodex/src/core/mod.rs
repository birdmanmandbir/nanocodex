...
pub(crate) const MEMORY_REVIEW_CHECKPOINT: &str = concat!(
    ...
    "This fixed Nanocodex control text is not user-authored. Treat the preceding later user message as ",
...
impl ConfiguredAgent {
    ...
    pub(crate) fn from_config_with_session(
        ...
        let openai = if let Some(runtime) = runtime {
            runtime.openai()
        } else {
        ...
        let mut tools = runtime
            .map_or_else(Tools::builder, |runtime| runtime.tools().into_builder())
        ...
        if let Some(runtime) = runtime {
            builder = builder.model(runtime.model());
        }
