...
const REVIEW_ASSETS_ENV: &str = "NANOCODEX_REVIEW_ASSETS";
...
impl ReviewAssets {
    pub(crate) fn availability() -> Result<AssetAvailability, AssetError> {
        ...
        let path = install_path(&nanocodex_home()?);
        if path.exists() {
    ...
    pub(crate) async fn download() -> Result<Self, AssetError> {
        ...
        let nanocodex_home = nanocodex_home()?;
        let review_root = nanocodex_home.join("review");
        ...
        let destination = install_path(&nanocodex_home);
        ...
        let archive_name = format!("nanocodex-review-v{version}.tar.gz");
        let archive =
...
fn nanocodex_home() -> Result<PathBuf, AssetError> {
    if let Some(path) = env::var_os("NANOCODEX_HOME").filter(|value| !value.is_empty()) {
    ...
    env::var_os("HOME")
        ...
        .map(PathBuf::from)
        .map(|home| home.join(".nanocodex"))
        .ok_or(AssetError::HomeUnavailable)
}

fn install_path(nanocodex_home: &Path) -> PathBuf {
    nanocodex_home
        .join("review")
...
fn validate_manifest(
    ...
    let compatible_nanocodex = manifest.nanocodex.version == current_version
        || (kind.allows_symlinks() && manifest.nanocodex.version == "development");
    if !compatible_nanocodex {
        return Err(AssetError::NanocodexVersion {
            expected: current_version,
            actual: manifest.nanocodex.version,
        });
    }
...
struct AssetManifest {
    ...
    nanocodex: NanocodexCompatibility,
    entrypoint: String,
    files: Vec<AssetFile>,
}
...
pub(crate) enum AssetError {
    #[error("could not determine the Nanocodex directory; set NANOCODEX_HOME")]
    HomeUnavailable,
    #[error(
        "this development build cannot download release review assets; set NANOCODEX_REVIEW_ASSETS to an explicit development bundle"
    )]
    DevelopmentDownload,
    ...
    #[error("review assets target Nanocodex `{actual}`, but this binary is Nanocodex `{expected}`")]
    NanocodexVersion {
        expected: &'static str,
        actual: String,
    },
...
mod tests {
    use super::{
        ApiCompatibility, AssetError, AssetFile, AssetManifest, InstallKind,
        NanocodexCompatibility, ReviewAssets, extract, validate_directory,
    };
