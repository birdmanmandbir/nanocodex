...
use tact_crossterm::event::{Event, KeyCode, KeyEventKind, KeyModifiers, MouseEventKind};
use tact_ratatui::{
    Frame,
...
mod tests {
    ...
    use tact_ratatui::{Terminal, backend::TestBackend};
    ...
    fn auto_theme_redraws_for_system_changes_but_explicit_modes_ignore_them() {
        ...
        assert_eq!(
            ...
            tact_ratatui::style::Color::Rgb(0xEE, 0xEE, 0xEE)
        );
        ...
        assert_eq!(
            app.theme.code_background(),
            tact_ratatui::style::Color::Rgb(0x26, 0x26, 0x26)
        );
    }
    ...
    fn update_available_routes_to_the_primary_notification() {
        ...
        assert_eq!(update.render, super::RenderRequest::Immediate);
        assert!(
            rendered(&mut app, 80, 12)
                .contains("Update available · v1.2.3 · run `nanocodex update`")
        );
    }
