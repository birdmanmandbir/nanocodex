...
use tact_crossterm::event::{Event, KeyCode, KeyEventKind, KeyModifiers};
use tact_ratatui::{
    Frame,
...
impl MessageQueue {
    ...
    pub(super) fn focus_row(&mut self, row: u16, area: Rect) -> bool {
        if !area.contains(tact_ratatui::layout::Position::new(area.x, row)) {
            return false;
        }
...
mod tests {
    ...
    use tact_ratatui::{Terminal, backend::TestBackend, style::Color};
