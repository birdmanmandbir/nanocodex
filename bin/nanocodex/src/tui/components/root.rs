    ...
    fn update_available(version: Version) -> Self {
        ...
        Self {
            message: Line::from(vec![
                ...
                Span::styled("`nanocodex update`", Style::default().fg(Color::Reset)),
            ]),
            color: Color::Green,
            deadline: Instant::now() + BREADCRUMB_DURATION,
        }
...
fn is_left_click_in(event: &Event, area: Rect) -> bool {
    ...
    area.contains(tact_ratatui::layout::Position::new(mouse.column, mouse.row))
}
...
mod tests {
    ...
    fn text_column(buffer: &tact_ratatui::buffer::Buffer, row: u16, text: &str) -> u16 {
        let symbols = text
            .chars()
    ...
    fn displaced_release_over_the_composer_copies_without_drag_events() {
        ...
        assert_eq!(buffer[(left, 0)].fg, tact_ratatui::style::Color::Green);
    ...
    fn update_available_uses_the_success_frame_and_styles_version_and_command() {
        ...
        let message = "Update available · v1.2.3 · run `nanocodex update`";
