...
mod tests {
    ...
    #[test]
    fn oversized_lines_skip_syntax_highlighting() {
        let assets = assets();
        let syntax = syntax_for_token(&assets.syntaxes, "rust");
        let syntax_theme = theme();
        let mut highlighter = HighlightLines::new(syntax, &syntax_theme);
        let source = format!("let value = \"{}\";", "x".repeat(MAX_HIGHLIGHT_LINE_BYTES));

        let spans = line(&mut highlighter, &source, &assets.syntaxes);

        assert_eq!(spans.len(), 1);
        assert_eq!(spans[0].content, source);
