...
use std::{env, fmt, process::Command, str::FromStr};
...
pub(crate) fn detect_system_scheme() -> Option<ColorScheme> {
    if let Some(scheme) = env::var_os("NANOCODEX_COLOR_SCHEME")
        .and_then(|value| value.into_string().ok())
        .and_then(|value| parse_scheme(&value))
    {
        return Some(scheme);
    }
    platform_system_scheme().or_else(terminal_color_scheme)
}

fn parse_scheme(value: &str) -> Option<ColorScheme> {
...
#[cfg(target_os = "macos")]
fn platform_system_scheme() -> Option<ColorScheme> {
    let output = Command::new("defaults")
        .args(["read", "-g", "AppleInterfaceStyle"])
        .output()
        .ok()?;
    if output.status.success() && String::from_utf8_lossy(&output.stdout).trim() == "Dark" {
...
fn terminal_color_scheme() -> Option<ColorScheme> {
...
const SYSTEM_SCHEME_POLL_INTERVAL: Duration = Duration::from_secs(2);
...
#[cfg(test)]
mod tests {
    use super::{ColorScheme, SYSTEM_SCHEME_POLL_INTERVAL, Theme, ThemeMode, parse_scheme};
    use tact_ratatui::style::Color;
    ...
    #[test]
    fn system_theme_polling_is_bounded_without_spawning_commands_per_frame() {
        assert!(SYSTEM_SCHEME_POLL_INTERVAL <= std::time::Duration::from_secs(2));
    }

    #[test]
    fn explicit_color_scheme_is_case_insensitive() {
        assert_eq!(parse_scheme(" LIGHT "), Some(ColorScheme::Light));
        assert_eq!(parse_scheme("dark"), Some(ColorScheme::Dark));
        assert_eq!(parse_scheme("system"), None);
    }
