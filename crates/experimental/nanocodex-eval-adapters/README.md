...
| ARC-AGI-3 | installed `arc-agi-3-public-smoke` recipe pinned to the official toolkit and benchmarking agent; `ls20`, `tn36`, and `sb26` exercise keyboard, click, and mixed-control action/frame/scorecard plumbing with anonymous keys and a three-action cap. Their environment scores are evidence-only: an official-comparable run still requires the evaluator-owned per-frame model-turn topology |
| Agents' Last Exam | installed `agents-last-exam` initial recipe for four official CLI-compatible tasks spanning algorithmic optimization, spreadsheet classification, PDF/HTML extraction, and economics reproduction; preparation pins the framework, gated task-data revision, and amd64 image digest, while import separates candidate-visible inputs/software from hidden references and official scorers |
| Big Finance Bench, LifeSciBench, FrontierMath, OSWorld, BenchCAD, CTF, SEC-Bench, ExploitBench, ExploitGym, KernelGen, NanoGPT, PostTrainBench, MMMU Pro, gdp.pdf, AutomationBench, and Toolathlon | Not installed until a dedicated recipe or caller-supplied official `external` manifest exists |
