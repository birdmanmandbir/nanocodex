...
use crate::{
    MppVmEgressError, SecretDelivery, SecretGateway, SecretManager, SecretSpec, mpp_egress_layer,
...
struct SecretRequestPolicy {
    ...
    gateway: Option<Arc<SecretGateway>>,
}
    ...
    async fn authorize(
        ...
        if request.method() == Method::CONNECT {
            if self
                .gateway
                .as_ref()
                .is_some_and(|gateway| gateway.matches_connect(request.uri()))
            {
                return Ok(request);
            }
        ...
        if self
            .gateway
            .as_ref()
            .is_some_and(|gateway| gateway.matches_request(request.uri()))
        {
            return Ok(request);
        }
    ...
    P: PaymentProvider + 'static,
{
    ...
    /// Routes guest base-URL variables through a scoped reverse gateway.
    ///
    /// The authenticated proxy remains active for clients that honor standard
    /// proxy variables. This compatibility route serves clients that use only
    /// an SDK-specific base URL. Both paths query the same live
    /// [`SecretPolicy`] and [`SecretManager`] on every request.
    #[must_use]
    ...
    pub async fn spawn(self) -> Result<VmEgress, VmEgressError> {
        ...
        let mut gateway_registration = None;
        let (proxy, initial_secrets) = if let Some(secrets) = secrets {
            let initial = secrets
                .policy
                .secrets(&secrets.context)
                .await
                .map_err(VmEgressError::Policy)?;
            gateway_registration = Some((
                secrets.context.clone(),
                Arc::clone(&secrets.policy),
                Arc::clone(&secrets.manager),
            ));
            let request_policy = SecretRequestPolicy {
                ...
                unmatched,
                gateway: secret_gateway.clone(),
            };
            (
                MppEgress::start_with_request_policy(provider, mpp_policy, request_policy)
                    .await
            ...
            (
                MppEgress::start(provider, mpp_policy)
                    .await
                    .map_err(VmEgressError::Proxy)?,
                Vec::new(),
            )
        ...
        if let (Some(gateway), Some((context, policy, manager))) =
            (secret_gateway, gateway_registration)
        {
            gateway
                .install(&mut lease, context, policy, manager, &initial_secrets)
                .map_err(VmEgressError::Gateway)?;
        } else {
...
pub enum VmEgressError {
    ...
    /// Scoped reverse-gateway setup failed.
    #[error(transparent)]
    Gateway(crate::SecretGatewayError),
...
mod tests {
    ...
    use super::*;
    use crate::{
        SecretConfigError, SecretGatewayError, SecretGuestConfig, SecretHttpMethod, SecretRef,
        SecretRequestRule,
    };
    ...
    async fn authorization_origin() -> (String, Arc<Mutex<Vec<String>>>, tokio::task::JoinHandle<()>)
    {
        ...
        (upstream, observed, server)
    }

    fn gateway_lease_token(gateway: &SecretGateway, route: &str, upstream: &str) -> String {
        assert!(route.starts_with("http://gateway.internal/internal/v1/secret-egress/"));
        assert!(!route.contains(upstream));
        assert!(
            gateway.matches_request(
                &format!("{route}/allowed/one")
                    .parse()
                    .expect("scoped route is an absolute URI"),
            )
        );
        assert!(
            !gateway.matches_request(
                &"http://gateway.internal/admin/v1/secrets"
                    .parse()
    ...
    async fn unified_proxy_injects_rotated_secrets_and_denies_other_paths() {
        ...
        let (upstream, observed, server) = authorization_origin().await;
        let manager = Arc::new(RotatingSecret {
            value: RwLock::new("alpha-secret".to_owned()),
        });
        let policy = Arc::new(RevocablePolicy {
            secrets: RwLock::new(Some(vec![scoped_secret(upstream.clone())])),
        });
        ...
        let allowed = client
            .get(format!("{upstream}/allowed/one"))
            .send()
            .await
            .unwrap();
        ...
        *manager.value.write().unwrap() = "beta-secret".to_owned();
        let allowed = client
            .get(format!("{upstream}/allowed/two"))
            .send()
            .await
            .unwrap();
        assert_eq!(allowed.status(), StatusCode::OK);
        let denied = client
            .get(format!("{upstream}/admin"))
            .send()
            .await
            .unwrap();
        ...
        *policy.secrets.write().unwrap() = None;
        let revoked = client
            .get(format!("{upstream}/allowed/three"))
            .send()
            .await
            .unwrap();
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn scoped_gateway_preserves_base_url_clients_with_live_policy() {
        let (upstream, observed, server) = authorization_origin().await;
        let manager = Arc::new(RotatingSecret {
            value: RwLock::new("alpha-secret".to_owned()),
        });
        ...
        let gateway = Arc::new(SecretGateway::new("http://gateway.internal").unwrap());
        let egress = VmEgress::builder(NoPayments)
            .secrets(
                EgressContext::new("agent-1", "principal-1"),
                policy.clone(),
        ...
        let route = lease
            ...
            .unwrap()
            .clone();
        let token = gateway_lease_token(&gateway, &route, &upstream);
        ...
        let response = gateway
            .forward(
                ...
                "test",
                "allowed/one",
                http::Request::get("/?query=retained")
                    .body(Vec::new())
                    .unwrap(),
            )
            .await
        ...
        *manager.value.write().unwrap() = "beta-secret".to_owned();
        let response = gateway
            .forward(
                ...
                "test",
                "allowed/two",
                http::Request::get("/").body(Vec::new()).unwrap(),
            )
            .await
            .unwrap();
        assert_eq!(response.status(), StatusCode::OK);
        assert!(matches!(
            gateway
                ...
                .await,
            Err(SecretGatewayError::RequestDenied)
        ));
        *policy.secrets.write().unwrap() = None;
        assert!(matches!(
            gateway
                ...
                .await,
            Err(SecretGatewayError::Policy(SecretPolicyError::Denied))
        ));
        assert_eq!(
            observed.lock().unwrap().as_slice(),
            ["Bearer alpha-secret", "Bearer beta-secret"]
        ...
        assert!(matches!(
            gateway
                ...
                .await,
            Err(SecretGatewayError::InvalidLease)
        ));
        server.abort();
    }
