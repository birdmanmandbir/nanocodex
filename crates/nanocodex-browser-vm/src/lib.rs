...
//!
//! This crate composes two otherwise independent components:
//!
//! - [`nanocodex_browser`] owns typed browser actions, diagnostics, artifacts,
//!   authentication policy, and the ordinary model-facing browser tool.
//! - [`nanovm`] owns the private VM, network helper, immutable root disk, and
//!   provider-neutral egress capability.
//!
...
impl BrowserVmBuilder {
    ...
    ///
    /// For example, the repository's proof VMM is selected with
    /// `.vmm_arg("--vmm")`. A dedicated VMM binary normally needs no
    /// preceding argument.
    #[must_use]
