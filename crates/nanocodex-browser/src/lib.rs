//! Headless browser control exposed as one ordinary Nanocodex tool.
//!
...
use nanocodex_tools::{
    ImageDetail, Tool, ToolContext, ToolDefinition, ToolExecution, ToolInput, ToolOutputContent,
    ToolResult, schema_for,
};
...
impl Tool for BrowserTool {
    ...
    async fn execute(&self, input: ToolInput, _context: ToolContext<'_>) -> ToolResult {
        ...
        if content.is_empty() {
            ...
            Ok(ToolExecution::json_content_with_value(
                &result,
                &code_mode_result,
                content,
            ))
