...
use nanocodex_tools::{
    DEFAULT_TOOL_OUTPUT_TOKENS, ToolContext, ToolOutputBody, ToolRuntime, Tools,
};
...
async fn managed_browser_executes_code_mode_against_chromium() -> Result<()> {
    ...
    assert!(
        content
            .iter()
            .any(|item| matches!(item, nanocodex_tools::ToolOutputContent::InputImage { .. }))
    );
...
fn execution_text(output: &ToolOutputBody) -> Result<&str> {
    ...
    content
        ...
        .find_map(|content| match content {
            nanocodex_tools::ToolOutputContent::InputText { text } => Some(text.as_str()),
            nanocodex_tools::ToolOutputContent::InputImage { .. }
            | nanocodex_tools::ToolOutputContent::InputAudio { .. } => None,
        })
