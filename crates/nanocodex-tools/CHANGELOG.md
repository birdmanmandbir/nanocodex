...
- Consolidate the heterogeneous registry, built-ins, shell lifecycle, Code
  Mode, MCP transports, authenticated discovery, and `tool_search`.
- Rebase implementations on the public tool contract in
  `nanocodex-oai-api`; MCP remains enabled on normal native builds.
