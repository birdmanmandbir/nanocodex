...
Nanocodex releases all public Rust crates, the CLI, and the JavaScript package
in lockstep. The ordered Rust package set is authoritative in
[`scripts/release-crates.txt`](../scripts/release-crates.txt); the release gate
checks it against Cargo metadata so a publishable package cannot be omitted.
The
...
- The root `CHANGELOG.md` is generated by `git-cliff` from every commit, while
  each published crate carries its own path-filtered
...
3. Generate the committed changelog after all intended feature commits are in
   ...
   `cargo-release`, refreshing the root changelog and every crate-specific
...
5. publishes the packages in `scripts/release-crates.txt` to crates.io in
   dependency order;
...
to publish every package in `scripts/release-crates.txt`. Configure `nanocodex`
on npm with the
