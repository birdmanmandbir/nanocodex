...
export class ApiClient {
  ...
  async review(signal?: AbortSignal): Promise<ReviewSession> {
    ...
    if (session.protocol_version !== REVIEW_PROTOCOL_VERSION) {
      throw new ApiError(
        "invalid_response",
        `This review UI supports protocol ${REVIEW_PROTOCOL_VERSION}, but Nanocodex returned ${session.protocol_version}.`,
      );
    }
    return session;
  }
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new ApiError("invalid_response", `Nanocodex returned an invalid ${path} response.`, response.status);
    }
  }
}
...
async function responseError(response: Response): Promise<ApiError> {
  ...
  const message = payload.error?.trim() || `Nanocodex returned HTTP ${response.status}.`;
