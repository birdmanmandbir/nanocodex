...
export class ReviewApp {
  ...
  render() {
    document.title = `${this.bootstrap.title} · Nanocodex`;
    this.root.innerHTML = `
      <div class="review-shell">
        ...
        <div id="finished" class="finished" hidden>
          <div><span>✓</span><h2>Review submitted</h2><p>You can return to Nanocodex.</p></div>
        </div>
      </div>`;
  ...
  async selectRange(
    ...
    try {
      ...
      if (payload.generation !== this.bootstrap.generation || !rangesEqual(payload.selected_range, range)) {
        this.showRangeError(
          "Nanocodex returned a page for a different review generation or range.",
          range,
  ...
  private installPage(page: ReviewPage) {
    ...
    const cacheKey = `nanocodex-review-${page.generation}-${rangeKey(page.selected_range)}`;
  ...
  private async pollQuestions() {
    ...
    try {
      ...
      if (!stored) {
        ...
        if (thread) {
          failQuestion(thread, operation.request, "Nanocodex did not retain this question. Ask again to retry.");
          if (thread.itemId) this.refreshItem(thread.itemId);
        }
        this.agentOperation = undefined;
        this.syncAgentControls();
      ...
      this.agentOperation = undefined;
      this.syncAgentControls();
      if (stored?.status === "idle") this.announceAgent("Nanocodex answered the question.");
      else if (stored?.status === "cancelled") this.announceAgent("Question cancelled.");
      else if (stored?.status === "error") {
        this.announceAgent(`Nanocodex could not answer the question: ${stored.error ?? "Unknown error"}`);
      }
    } catch (error) {
  ...
  private renderOverviewState() {
    ...
    state.innerHTML = `
      ...
      <span>Ask Nanocodex’s root agent to explain and visualize the change when you need it.</span>
  ...
  private async loadOverview(restoring = false) {
    ...
    if (state) {
      ...
      state.innerHTML = `
        ...
        <span>Nanocodex’s root agent is inspecting the selected range and surrounding code.</span>`;
    }
    ...
    try {
      ...
      if (payload.generation !== page.generation
        ...
        this.showOverviewError("Nanocodex returned an overview for a different review range.");
        return;
      }
  ...
  private startQuestion(
    ...
    this.announceAgent(`Nanocodex is answering a question about ${thread.path}, ${formatRange(thread.startLine, thread.endLine)}.`);
    this.refreshItem(thread.itemId);
    this.syncAgentControls();
  ...
  private async sendQuestion(
    ...
    try {
      ...
      if (payload.generation !== page.generation
        ...
        throw new ApiError(
          "invalid_response",
          "Nanocodex returned an invalid answer for this review range.",
        );
      }
      finishQuestion(thread, request, payload.answer);
      this.announceAgent(`Nanocodex answered the question about ${thread.path}, ${formatRange(thread.startLine, thread.endLine)}.`);
    } catch (error) {
      ...
      this.announceAgent(
        cancelled
          ? "Question cancelled."
          : `Nanocodex could not answer the question: ${errorMessage(error)}`,
      );
  ...
  private questionThreadElement(thread: QuestionThread) {
    ...
    element.innerHTML = `
      <header class="agent-thread-heading">
        <div><strong id="${headingId}">Ask Nanocodex <span aria-hidden="true">✨</span></strong><span id="${linesId}">${lineLabel} ${escapeHtml(formatRange(thread.startLine, thread.endLine))}</span></div>
    ...
    for (const message of thread.messages) {
      ...
      entry.innerHTML = `<strong>${message.role === "reviewer" ? "You" : "Nanocodex"}</strong><div class="thread-markdown"></div>`;
    ...
    if (thread.turn.kind === "asking") {
      ...
      turn.innerHTML = `<span class="thread-spinner" aria-hidden="true">${icon("sparkles")}</span><span>${thread.turn.stopping ? "Stopping…" : "Nanocodex is answering…"}</span><button class="button quiet" data-thread-stop ${thread.turn.stopping ? "disabled" : ""}>${thread.turn.stopping ? "Stopping…" : "Stop"}</button>`;
  ...
  private async cancel() {
    ...
    const finished = this.root.querySelector<HTMLElement>("#finished");
    if (!finished) return;
    finished.innerHTML = "<div><span>✓</span><h2>Review cancelled</h2><p>You can return to Nanocodex.</p></div>";
