...
const server = Bun.serve({
  ...
  async fetch(request) {
    ...
    if (request.method === "POST" && url.pathname === "/api/question") {
      ...
      const answer = `This thread is anchored to \`${body.path}:${body.start_line}${body.end_line === body.start_line ? "" : `-${body.end_line}`}\`. In a real review, Nanocodex asks a sub-agent to inspect the surrounding code and answer with that context.`;
...
console.log(`Nanocodex review UI: http://localhost:${server.port}`);
